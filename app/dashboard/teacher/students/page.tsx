...
const CustomCheckbox = ({ checked, onChange, className, disabled }: { checked: boolean; onChange: () => void; className?: string; disabled: boolean }) => {
  return (
    <div
...
export default function AttendanceManagement() {
  ...
  const [isSunday, setIsSunday] = useState(false); // ✅ New state for Sunday check

  useEffect(() => {
    const dayOfWeek = selectedDate.getDay(); // 0 is Sunday, 6 is Saturday
    const isTodaySunday = dayOfWeek === 0;

    setIsSunday(isTodaySunday); // ✅ Set the state for Sunday

    if (isTodaySunday) {
      setIsLoading(false);
      setAttendanceStats({ total: 0, present: 0, absent: 0, leave: 0, rate: 0 });
      setStudents([]);
      toast.info("Attendance cannot be marked on Sundays.");
      return;
    }
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                disabled={isSunday || isLoading} // ✅ Disable on Sunday
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow>
                   ...
                   <TableRow>
                    <TableCell colSpan={7} className="text-center text-gray-500 py-12 font-semibold text-xl">
                    ...
                    <TableRow key={student.id} className={cn(
                      ...
                      <TableCell className="text-center">
                        <CustomCheckbox
                          ...
                          disabled={isSunday} // ✅ Disable on Sunday
                        />
                      </TableCell>
                      <TableCell className="text-center">
                        <CustomCheckbox
